...
/**
 * In-app notification email (Module 13). Mirrors an in-app notification into the
 * recipient's inbox, with an optional deep-link button back into the app.
 * @param {{ title?: string, body?: string, link?: string }} notification
 * @param {{ firstName?: string }} recipient
 */
export function notificationEmail(notification, { firstName } = {}) {
  const name = escapeHtml(firstName || 'there');
  const title = escapeHtml(notification?.title || 'You have a new notification');
  const body = notification?.body ? escapeHtml(notification.body) : '';
  // Build an absolute URL from the client base + the notification's link path.
  const url = notification?.link
    ? `${env.CLIENT_URL.replace(/\/$/, '')}/${String(notification.link).replace(/^\//, '')}`
    : null;
  const safeUrl = url ? escapeHtml(url) : null;
  const subject = `${env.EMAIL_FROM_NAME}: ${notification?.title || 'New notification'}`;

  const bodyBlock = body
    ? `<tr>
              <td style="padding:0 32px 16px 32px;">
                <div style="border-left:3px solid #2563eb;background:#f8fafc;padding:12px 16px;border-radius:0 8px 8px 0;font-size:14px;line-height:1.6;color:#374151;white-space:pre-wrap;">${body}</div>
              </td>
            </tr>`
    : '';

  const buttonBlock = safeUrl
    ? `<tr>
              <td align="center" style="padding:8px 32px 24px 32px;">
                <a href="${safeUrl}" style="display:inline-block;background:#2563eb;color:#ffffff;text-decoration:none;font-size:15px;font-weight:600;padding:12px 28px;border-radius:8px;">Open in DocFlow AI</a>
              </td>
            </tr>`
    : '';
...
<html lang="en">
          ...
          <table role="presentation" width="480" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:12px;overflow:hidden;border:1px solid #e4e7eb;">
            ...
            <tr>
              <td style="padding:8px 32px 0 32px;">
                <h1 style="margin:0 0 12px 0;font-size:20px;line-height:1.3;color:#111827;">${title}</h1>
                <p style="margin:0 0 16px 0;font-size:15px;line-height:1.6;color:#374151;">
                  Hi ${name}, you have a new notification on DocFlow&nbsp;AI.
                </p>
              </td>
            </tr>
            ${bodyBlock}
            ...
            <tr>
              <td style="padding:0 32px 28px 32px;border-top:1px solid #e4e7eb;">
                <p style="margin:16px 0 0 0;font-size:12px;line-height:1.6;color:#9aa5b1;">
                  You're receiving this because you have notifications enabled in DocFlow&nbsp;AI.
                </p>
              </td>
            </tr>
          </table>
  ...
  const text = [
    ...
    url ? '' : null,
    url ? `Open in DocFlow AI: ${url}` : null,
    '',
    '— DocFlow AI',
  ]
    .filter((line) => line !== null)
